...
module.exports = function(Questionpackages) {
    ...
    Questionpackages.getPackages2 = function (req, cb)
  {
    try
    {
      ...
      // if(reqObject.accessToken)
      // {
        userModel.findOne({where:{id:31},fields:{id:true,country_id:true,packages:true}},function(err,userInfo)
        {
          if(err)
          {
             cb(null,{status:"fail",message:"Error while Getting userinfo "})
          ...
          else
          {
            if(userInfo)
            {
              categoryModel.find({where:{status:1},fields:{id:true,iconImage:true}},function(err,cateInfo){
