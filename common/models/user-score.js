...
let rigs = require('./user-rings.js');
...
console.log("=========winner ===========================")
    ...
    Userscore.answerGivenByPlayer = function (req, cb)
    {
        try
        {
	console.log("=========Answer Given By Player ===========================")
    ...
    Userscore.updateUserRing = function (req, cb)
    {
        try
        {
console.log("=========update User Rings ===========================")

            let reqObject = req.res.req;
            let aData = JSON.parse(reqObject.body.data);
			//let saveGames =  app.models.save_games
			//let userRings =  app.models.user_rings
            //let aData =  {gameId: 235030,answerGiven:0,category_id:1,childId:163884,timeTaken:10}
			setAns(aData);
			saveGame(aData);
			updateRingsData(aData)
    ...
    Userscore.remoteMethod(
        'updateUserRing', {
            accepts: [{arg: 'req', type: 'object', http: function(req){return req}}],
            returns: [{arg: 'data',type: 'Object'}],
            http: {path: '/updateUserRing',verb: 'post'}
        });   
    ...
    async  function leaderBoard(info)
    {
          ...
          async.waterfall([
            ...
            function(sortedPointArray,callbackWater) 
            {
              ...
              async.eachSeries(sortedPointArray, function(value, callback)
              {
				...
				if(scoreINfo.toJSON().user_teams){
		...
		//console.log("userScoreInfo=====================><><><>",userScoreInfo)
                if(userScoreInfo.length > 0 )
                {
                  if(userScoreInfo[0].toJSON().user_teams)
                  {
                    let finalArray =[];
                    let profilePic="";
		    try{	
		    ...
		    catch(e)
		    {
			 reject(0)
                  ...
                  {
			console.log("Enter to the dragon")
                    reject(0)
                  }
                }
                else
 ...
 function compare_qty(a,b)
   {
	...
	function saveGame(aData)
    {
        
	let Savedgames = app.models.saved_games;
	...
	function updateRingsData(aData)
    {
		//console.log("update1111=====",aData);
        try
        {
