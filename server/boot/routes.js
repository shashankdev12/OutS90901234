            ...
            users.findOne({ where: { id: token.userId },fields:{id:true,username:true,email:true,userType:true,country_id:true,countryCode:true,packages:true} }, function (err, userDat) {
              ...
              else
              {
                ...
                else
                {
                  ...
                  req.session.regionName = "UK";
                  req.session.regionCode = "UK";
		              req.session.loginType = "superAdmin";
                  req.session.loginName = "superAdmin";
                  req.session.regionId = 4;
                  //req.session.packValG=userDat.packages;
                  //res.redirect('./reviewQuestions/1/0');
              ...
              else
              {
                countriesModel.findOne({where :{id:userDat.countryCode}},function(err,countryData){
                  ...
                  else
                  {
                    ...
                    req.session.packValG=userDat.packages;
		    req.session.loginName = userDat.username;
                    req.session.regionId = countryData.id;
                    res.redirect('./reviewQuestions/1/0');
                }
              })
              }
    ...
    try
    {
      if (req.session.token)
      {
        //console.log("req.session<<<<<<<<<>>>>>>>>>>>>>>>>>",req.session)
          ...
          {
            //console.log(data.addontopALL);
            res.render('./adminDashboard', 
                  {totalLicences:data.licenceCount,userCount:data.userCount,juniorCount:data.jCount,
                  teenCount:data.tCount,adultCount:data.aCount,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,
                  userLicenLast:data.userLicenLast,userGames:data.userGames,totalChilds:data.totalChilds,
                  addontopten:data.addontopten.reverse(),addontopALL:data.addontopALL.reverse(),
                  regionId:req.session.regionId
                });
          });
      }
        ...
        //////console.log('req.session.adminUserType= 2',req.session.adminUserType)
          ...
          adminController.data.getDashboardP(req, res, function (err, data)
          {
            res.render('./adminDashboard', {totalLicences:data.licenceCount,userCount:data.userCount,juniorCount:data.jCount,
                                    teenCount:data.tCount,adultCount:data.aCount,
...
/* ====================== User List =========================== */
  ...
  app.get('/users/:id/:odr/:type', function (req, res, cb) {
    if (req.session.token && req.session.adminUserType == 1)
    {
        ...
        {
           //console.log(data);
          res.render('./users', {userData: data.userData,paginator:data.userCount,page:req.params.id,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,orderby:req.params.odr,type:req.params.type});
        });
    ...
    if (req.session.token && req.session.adminUserType == 1)
    {
        adminController.data.getUserDetails(req, res, function (err, data)
        {
          ...
          res.render('./userDetails',{userData:data.userDetails ,devicesAttached:data.devicesAttached,
            ...
            teamInfo:data.teamInfo,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,newCategory:data.newCategory,gameStart:data.game_start,gameEnd:data.game_end});
        });      
    }
    else
    {
...
/* ====================== Set/Add Age ===================== */
  ...
  app.get('/questions/:id', function (req, res, cb)
  {
    if (req.session.token)
    {
        ...
        adminController.data.questions(req, res, function (err, data)
        {
          ...
          ,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,package_id:packages,countryCode2:req.session.regionCode,
  ...
  /* ====================== Edit Questions  ========================= */
  app.get('/editQuestions/:id/:type/:regionType/:region/:checkQuestionId', function (req, res, cb)
  {
    if (req.session.token)
    {
      let packages = 0;
      if(req.session.adminUserType != 1)
      {
        ...
        adminController.data.editQuestions(req, res, function (err, data)
        {
            ...
            let questionsModelUk =  app.models.questions_UK;
              questionsModelUk.findOne({where:{questionMasterId:req.params.id}},function(err,ukData){
                //console.log("data.subCategory",err)
            //console.log("data.subCategory",ukData)
          ...
          regionId:req.session.regionId,
            userStatus:req.session.adminUserType,
          regionId:req.session.regionId,package_id:packages,regionsession:req.session.region
            ,type:req.params.type,regionType:req.params.regionType,question_id:req.params.id,
            questionsMain:ukData,regionCode:req.session.regionCode});
          })
        })
    ...
    // {
      ...
      adminController.data.getQuestions1(req, res, function (err, data)
      {
        ...
        res.render('./viewQuestions',{questionsData: data.questionsData
          ...
          userStatus:req.session.adminUserType,
          regionId:req.session.regionId,
          page:req.params.id,sessionRegion:req.session.regionCode,
    ...
    if (req.session.token)
    {
      adminController.data.getFreePlayQuestions(req, res, function (err, data)
      {
        ...
        res.render('./viewFreeplayQuestions',{questionsData: data.questionsData,paginator:data.count,
          ...
          ,page:req.params.id,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,
          userStatus:req.session.adminUserType,
          regionId:req.session.regionId,
          regionId:req.session.regionId});
...
app.post('/getUserCount', function (req, res, cb) {
    //console.log("enter");
    adminController.data.getAjaxUserCount(req, res, function (err, data)
    {
      if (err)
      {
        cb(null, {status: 0,message: err.message,cityInfo:null});
...
app.post('/getUniqueUser', function (req, res, cb) {
  
  adminController.data.getAjaxUniqueUserCount(req, res, function (err, data)
  {
    if (err)
    {
      cb(null, {status: 0,message: err.message,cityInfo:null});
...
app.post('/getUniqueGames', function (req, res, cb) {
  
  adminController.data.getAjaxGamePlayed(req, res, function (err, data)
  {
    if (err)
    {
      cb(null, {status: 0,message: err.message,cityInfo:null});
  ...
  app.get('/packages/:id', function (req, res, cb) {
    if (req.session.token && req.session.adminUserType == 1)
    {
      adminController.data.getPackages(req, res, function (err, data)
      {
          ...
          countryInfo:data.countryInfo,categoryInfo:data.categories,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,ageData:data.ageInfo});
      });
    }
    else
    {
...
/* ====================== Set/Add package ===================== */
  ...
  app.get('/addCategory', function (req, res, cb)
  {
    if (req.session.token && req.session.adminUserType == 1)
    {
     adminController.data.getCategories(req, res, function (err, data)
     {
       ...
       res.render('./addCategory',{grp:data.grp,categoryData: data.categoryData,speficData:data.specificData,countries:data.countries,userStatus:req.session.adminUserType,
          regionId:req.session.regionId});
     });
   }
   else
 ...
 app.get('/editCategory/:id', function (req, res, cb)
  {
    if (req.session.token && req.session.adminUserType == 1)
    {
     adminController.data.getEditCategories(req, res, function (err, data)
     {
       console.log('datad==================================.',data.categoryData);
       res.render('./editCategory',{grp:data.grp,categoryData: data.categoryData
        ,countries:data.countries,userStatus:req.session.adminUserType,
          regionId:req.session.regionId});
     });
   }
   else
...
/* ====================  View Categories ============================ */
  ...
  app.get('/viewCategories/:id', function (req, res, cb)
  {
    ...
    // {
     ...
     {
       ////console.log('datad==================================.',data);
       res.render('./viewCategories',{grp:data.grp,categoryData: data.categoryData,speficData:data.specificData,countries:data.countries,userStatus:req.session.adminUserType,
          regionId:req.session.regionId});
     });
  //  }
  //  else
 ...
 app.get('/viewRTag/:id', function (req, res, cb)
  {
    // if (req.session.token && req.session.adminUserType == 1)
    // {
     adminController.data.getRCategories(req, res, function (err, data)
     {
       ////console.log('datad==================================.',data);
       res.render('./viewRTag',{tags:data.tags,grp:data.grp,categoryData: data.categoryData,speficData:data.specificData,
        countries:data.countries,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,countryId:req.params.id});
     });
  //  }
  //  else
  //  {
    ...
    tempQuestionModel.find({},function(err,data){
      ...
      else
      {
        ...
        async.eachSeries(data, function(fields, callback)
        {
          ...
          if(groupExist == 1)
          {
            console.log("inside grou records")
            countriesModel.find({},function(err,allCountry)
            {
              let y=0;
              let countries=fields.region;
              async.eachSeries(allCountry, function(fields2, checkAllCallback)
              { 
                console.log("countries",fields2.language)
                let questionModel = app.models["questions_"+fields2.language];
                
                y++
                if(y == allCountry.length)
                {
                  questionModel.findOne({where:{questionMasterId:fields.questionMasterId}},function(err,questionExist){
                    if(questionExist)
                    {
                      
                        countries = countries+","+fields2.id;
                      
                      
                      countriesModel.findOne({where:{id:fields.region}},function(err,mainE)
                      {
                        console.log("mmmmmmmm")
                          questionModel.updateAll({questionMasterId:fields.questionMasterId},
                            {region:questionExist.region+","+fields.id},function(err,udateOO){
                              console.log("----","questions_"+mainE.language) 
                              questionModel = app.models["questions_"+mainE.language];
                              ...
                              obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                                ...
                                region:countries,question:fields.question,answer1:fields.answer1,
                    ...
                    else
                    {
                      countriesModel.findOne({where:{id:fields.region}},function(err,mainE)
                      {
                        ...
                        obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                          pack_ID:fields.pack_ID,time_Allowed:fields.time_Allowed,age_id:fields.age_id,
                          region:countries,question:fields.question,answer1:fields.answer1,
                          answer2:countries,answer3:fields.answer3,answer4:fields.answer4,
                          correct_Answer:fields.correct_Answer,image_URL:fields.image_URL,video_URL:fields.video_URL,
                          fileType:fields.fileType,sound_URL:fields.sound_URL
                          ,creditBy:fields.creditBy,status:fields.status,
                          created:new Date,hint:fields.hint,questionState:fields.questionState,
                ...
                else
                {
                  ...
                  questionModel.findOne({where:{questionMasterId:fields.questionMasterId}},function(err,questionExist){
                    if(questionExist)
                    {
                      ...
                      questionModel.updateAll({questionMasterId:fields.questionMasterId},
                        {region:questionExist.region+","+fields.region},function(err,udateOO){
                        checkAllCallback()
                      })
                    }
                    else
                    {
                      console.log("exist callllll");
                      checkAllCallback()
                    }
          ...
          else
          {
                  ...
                  let daatRegion =  fields.region.split(",");let y=0;
                  async.eachSeries(daatRegion, function(fields2, callback2)
                  { 
                    console.log("calling",fields2)
                    countriesModel.findOne({where:{id:fields2}},function(err,data1)
                    {
                      console.log(data1)
                      let questionModel = app.models["questions_"+data1.language];

                      y++
                      if(daatRegion.length ==  y)
                      {
                        obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                          pack_ID:fields.pack_ID,time_Allowed:fields.time_Allowed,age_id:fields.age_id,
                      ...
                      else
                      {
                        obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                          ...
                          correct_Answer:fields.correct_Answer,image_URL:fields.image_URL,video_URL:fields.video_URL,
                          fileType:fields.fileType,sound_URL:fields.sound_URL,creditBy:fields.creditBy,
                          created:new Date,questionActiveStatus:fields.questionActiveStatus
                          ,questionMasterId:fields.questionMasterId,modified:new Date(),hint:fields.hint}
                        questionModel.create(obj,function(err,addedQuestionInfo)
                        {
                          if(err)
                          {
...
app.get('/ageCategoryStats', function (req, res, cb) {
  ...
  if (req.session.token && req.session.adminUserType == 1)
  {
    adminController.data.getCateAgeStats(req, res, function (err, data)
    {
      ...
      let arr = []
      for(let i=0;i<data.dataInfo.length;i++)
      {
        if(data.dataInfo[i].toJSON().categories)
        {
          let obj={}
          if(arr.length>0)
          {
            let xyz = arr.filter(a=>a.category_id == data.dataInfo[i].category_id)
            if(xyz.length>0)
            {
              if(arr[xyz[0].tempid] != undefined)
              {
                arr[xyz[0].tempid].info.push({age:data.dataInfo[i].toJSON().age_categories.age,
                  questionCount:data.dataInfo[i].questionCount, answerCount:data.dataInfo[i].answerCount})
              }
            }
            else
            {
              obj = {tempid:i,category_id:data.dataInfo[i].category_id,category:data.dataInfo[i].toJSON().categories.category,
                info:[{age:data.dataInfo[i].toJSON().age_categories.age,
                questionCount:data.dataInfo[i].questionCount, answerCount:data.dataInfo[i].answerCount}]}
              arr.push(obj);
            }
          }
     ...
     // //console.log(arr)
      ...
      res.render('./ageCategoryStats',{cateInfo:arr,userStatus:req.session.adminUserType,
          regionId:req.session.regionId})
    });
  }
  else
...
app.get('/redeemCodeView/:id', function (req, res, cb) {
    if (req.session.token && req.session.adminUserType == 1)
    {
        ...
        {
          ////console.log("categoriescategories",data.categories);
	        res.render('./redeemCodeView',{redeemCodeData: data.redeemCodeData,
            paginator:data.redeemCodeCount,page:req.params.id,categories:data.categories,
            userStatus:req.session.adminUserType,
          regionId:req.session.regionId});
        })
    }
 ...
 app.get('/newCategory/:id', function (req, res, cb)
 {
   if (req.session.token && req.session.adminUserType == 1)
   {
     adminController.data.getNewCategories(req, res, function (err, data)
     {
       //console.log(data.specificData);
       res.render('./viewNewCategory',{categoryData: data.categoryData,speficData:data.specificData,
        countries:data.countries,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,countryId:req.params.id});
     });
   }
...
app.get('/updateCategory/:name', function (req, res, cb)
 {
   if (req.session.token && req.session.adminUserType == 1)
   {
     adminController.data.getCategoriesOnly(req, res, function (err, data)
     {
 ...
 app.get('/deleteTag/:name', function (req, res, cb)
 {
   if (req.session.token && req.session.adminUserType == 1)
   {

     let catemodel = app.models.region_categories;
     let categories =  app.models.categories;
     let ds1 = categories.dataSource; 

     ds1.connector.query('SELECT * FROM region_categories WHERE tag like "%'+req.params.name+'%" order by tag_index asc', function (err, data)	
     {
        //console.log("data",data);
        let x=0;
        async.eachSeries(data, function(field, cback)
        {
          ...
          if(data.length ==x)
          {
            let fieldTags =  field.tag.split(',');
            ////console.log("fieldTags",fieldTags)
            if(fieldTags.includes(req.params.name))
            {
                const index = fieldTags.indexOf(req.params.name);
                if (index > -1) { // only splice array when item is found
                  fieldTags.splice(index, 1); // 2nd parameter means remove one item only
                }
            }
            ////console.log("fieldTags.toString()",fieldTags.toString())
            //console.log(fieldTags)
            catemodel.updateAll({id:field.id},{tag:fieldTags.toString()},function(err,data){
              //console.log(err);
              res.redirect('/viewCategories/0');
            })
          }
...
app.post('/addTagIndex', function (req, res, cb)
 {
      adminController.data.addTagIndexD(req, res, function (err, data)
      {
          res.write(JSON.stringify(data));
          res.end();
      });
...
app.get('/viewExportQuestion/:type', function (req, res, cb) 
{
   if (req.session.token && req.session.adminUserType == 1)
   {
    ...
    let ds1 = countries.dataSource;
    let ds2 = categories.dataSource;
    let ds3 = question_packages.dataSource;
    ...
    ds1.connector.query(countriesProcedure, (error, countriesData, fields) => {
      if (error) {
      ...
      else
      {
        ...
        ds2.connector.query(categoriesProcedure, (error, categoriesData, fields) => {
          if (error) 
          {
            return //console.error(error.message);
          }
          else
          {
            ...
            ds3.connector.query(packagesProcedure, (error, packagesData, fields) => {
              if (error) 
              {
                return //console.error(error.message);
              }
              else
              {
                ...
                res.render('./viewExportQuestion', {type:req.params.type,userStatus:req.session.adminUserType,
          regionId:req.session.regionId,categories:allData.categories,country:allData.countries,packages:allData.packages});
...
//   });
  ...
  app.get('/checkedQuestions/:id', function (req, res, cb) 
  {
    adminController.data.deleteFromCheckedQuestions(req, res, function (err, data)
    {
      req.flash('notify', data.message);
      res.redirect('/wrongQuestions/0');
    })
  });

 app.get('/wrongQuestions/:page', function (req, res, cb) {
      if (req.session.token)
      {	
      	adminController.data.getWrongQuestions(req, res, function (err, data)
      	{
        	//console.log("nnnn",data)
       		 req.session.editType=2;
        
        	res.render('./wrongQuestions',{userStatus:req.session.adminUserType,
          regionId:req.session.regionId
                                        ,tempQuestionInfo:data.info,
                                        category:data.category,
                                        type:req.params.type,
                                        ...
                                        sessionRegion:req.session.regionCode,
