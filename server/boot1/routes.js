...
/* ====================== User List =========================== */
  ...
  app.get('/users/:id', function (req, res, cb) {
    if (req.session.token && req.session.adminUserType == 1)
    {
    ...
    if (req.session.token && req.session.adminUserType == 1)
    {
        ...
        adminController.data.getUserDetails(req, res, function (err, data)
        {
          ...
          res.render('./userDetails',{userData:data.userDetails ,devicesAttached:data.devicesAttached,
            childs:data.childs,packagesData:data.package,categoryData:data.categories,gameCount:data.gameCount,teamInfo:data.teamInfo,userStatus:req.session.adminUserType});
        });
...
/* ====================== Set/Add Age ===================== */
  ...
  app.get('/questions/:id', function (req, res, cb)
  {
    if (req.session.token)
    {
        ...
        adminController.data.questions(req, res, function (err, data)
        {
          ...
          ,userStatus:req.session.adminUserType,package_id:packages});
        })
  ...
  /* ====================== Edit Questions  ========================= */
  app.get('/editQuestions/:id', function (req, res, cb)
  {
    if (req.session.token)
    {
      let packages = 0;
...
/* ====================  Question List ============================ */
  ...
  app.get('/viewQuestions1/:category/:subCategory/:package/:likeQuestion/:id', function (req, res, cb) {
    if (req.session.token && req.session.adminUserType == 1)
    {
      ...
      {
        res.render('./viewQuestions',{questionsData: data.questionsData,paginator:data.questionsCount,category:data.category,packages:data.package,category_page_id:req.params.category,sub_page_category_id:req.params.subCategory,package_id:req.params.package,userStatus:req.session.adminUserType});
      });
    }
    else
    {
    ...
    // {
      ...
      adminController.data.getQuestions1(req, res, function (err, data)
      {
        ...
        res.render('./viewQuestions',{questionsData: data.questionsData
          ...
          questionStatus:req.params.questionStatus,questionState:req.params.questionState,userStatus:req.session.adminUserType,page:req.params.id});
      });
    // }
    // else
    // {
...
/* ===================  Detail Questions =========================== */
 ...
 app.get('/deleteQuestion/:id/:type', function (req, res, cb) {
   if (req.session.token)
   {
...
/* ===================== Edit Category =============================== */
 ...
 app.get('/copyQuestion/:id', function (req, res, cb)
 {
   if (req.session.token )
   {
        ...
        async.eachSeries(data, function(fields, callback)
        {
            ...
            if(x == data.length)
            {
              ...
              obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                ...
                created:new Date,hint:fields.hint,questionState:fields.questionState,questionActiveStatus:fields.questionActiveStatus,modified:new Date()}
            ...
            else
            {
              obj = { category_id:fields.category_id,sub_category_id:fields.sub_category_id,
                ...
                created:new Date,questionActiveStatus:fields.questionActiveStatus,modified:new Date()}
              questionModel.create(obj,function(err,addedQuestionInfo)
              {
                if(err)
                {
